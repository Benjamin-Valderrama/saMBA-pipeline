...
function display_usage() {
    ...
    echo "  -a, --analyse        Run reads quality check and alignment [uses kneaddata]."
    echo ""
...
cut -f1 $input | tail -n +2 | uniq | while read -r bioproject; do
    ...
    bash ${SCRIPTS_FOLDER}/analyse_project.sh -s ${output}/${bioproject} --run_dada2 >> ${output}/nohups/${bioproject}.log &
